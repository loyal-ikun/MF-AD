...
class Dfm(AnomalyModule):
    ...
    def training_step(self, batch: dict[str, str | Tensor], *args, **kwargs) -> None:
        ...
        batch["data"] = batch["data"].reshape(-1, 3, 40, 40)

        embedding = self.model.get_features(batch["data"]).squeeze()
    ...
    def validation_step(self, batch: dict[str, str | Tensor], *args, **kwargs) -> STEP_OUTPUT:
        ...
        if self.score_type == "fre":
            batch["anomaly_maps"], temp = self.model(batch["data"])
        elif self.score_type == "nll":
            temp = self.model(batch["data"])
        temp = temp.reshape(-1, 8)
        
        batch["pred_scores"] = torch.zeros(temp.shape[0]).cuda()
        for i in range(temp.shape[0]):
            for ii in range(8):
                batch["pred_scores"][i] += temp[i, ii]
        return batch
