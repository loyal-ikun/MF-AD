...
class Fastflow(AnomalyModule):
    ...
    def training_step(self, batch: dict[str, str | Tensor], *args, **kwargs) -> STEP_OUTPUT:
        ...
        hidden_variables, jacobians = self.model(batch["data"])
    ...
    def validation_step(self, batch: dict[str, str | Tensor], *args, **kwargs) -> STEP_OUTPUT:
        ...
        """
        del args, kwargs  # These variables are not used.
        shape0 = batch["data"].shape[0]

        batch["data"] = batch["data"].reshape(-1, 3, 40, 40)
        anomaly_maps = self.model(batch["data"])

        anomaly_maps = anomaly_maps.reshape(shape0, 8, 1, 40, 40)
